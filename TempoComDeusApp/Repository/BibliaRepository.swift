...
//        HTTP.get.request(url: .getLivros) { data, response, error in
//
//               if let error = error {
//                             print(error)
//                            completion([])
...
//                                    print(obj) } else if let obj = json as? [Any] {
...
//           }
    ...
    func getCapitulo(livro: String, cap: Int, completion: @escaping (Biblia) -> Void) {
      HTTP.get.request(url: .getCapitulo(abbrev: livro, cap: cap)) { data, response, error in
                 ...
                 if let error = error {
                        ...
                        let aux = Biblia(book: Livro(abbrev: ["pt": "", "en": ""],
                                                     name: "", author: "",
                                                     chapters: 0,
                                                     group: "",
                                                     testament: "",
                                                     version: ""),
                                         chapter: Capitulo(number: 0, verses: 0), verses: [Verso(number: 0, text: "")])
